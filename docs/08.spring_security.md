...
- `Member`, `MemberMapper`, `CustomUserDetails`, `CustomUserDetailsService`를 통해 사용자 인증 기반 체계가 구축된 후, 컨트롤러나 서비스 계층에서 현재 로그인한 사용자의 인증 정보를 꺼내 쓰는 3가지 표준 접근 방식을 적용하고 실제 비즈니스 API에 연동함.
- HTTP Basic 인증 메커니즘을 기반으로 로그인 및 내 정보 조회 실습을 수행하고, 07단원에서 임시 헤더(`X-Member-Id`)로 처리했던 게시글 등록 API를 스프링 시큐리티 인증 객체 기반으로 개편함.

### 2.4.1 인증 정보 접근 3대 방식
...
#### 2) 미인증 접근 차단 검증
- Auth 탭의 Type을 `No Auth`로 변경한 후 재요청 시, 401 Unauthorized 상태 코드가 반환되어 비인가 접근이 정확히 차단됨을 확인함.

### 2.4.3 로그인 사용자 정보를 활용한 게시글 등록 연동 실습
- 07단원 REST API 구현 시점에는 보안 인증 체계가 마련되지 않아, 클라이언트가 임의로 전송하는 HTTP 요청 헤더(`@RequestHeader("X-Member-Id")`)에 의존하여 게시글 작성자를 지정했음.
- 클라이언트가 헤더 값을 조작할 경우 타인의 명의로 게시글을 작성할 수 있는 심각한 보안 결함이 발생하므로, 스프링 시큐리티의 인증 객체(`@AuthenticationPrincipal`)로부터 검증된 회원 식별자를 획득하여 작성자로 바인딩하도록 개선함.

#### 1) 게시글 등록 컨트롤러 수정 (src/main/java/net/likelion/bebc25/sns/controller/PostRestController.java)
- 기존의 임시 헤더 파라미터(`@RequestHeader("X-Member-Id") Long memberId`)를 제거하고, 스프링 시큐리티가 인증 체계에서 보증한 `@AuthenticationPrincipal CustomUserDetails userDetails`를 주입받도록 수정함.
  ```java
...
- 요청 헤더 (Headers 탭)
  ...
  - 기존 실습에서 사용했던 `X-Member-Id` 헤더는 완전히 제거함.
- 요청 본문 (Body -> JSON)
  ```json
  {
...
- 결과 확인
  - 상태 코드: `201 Created`
  - 응답 헤더: `Location: /api/v1/posts/3` (생성된 게시글 URI)
  - 응답 본문:
    ```json
    {
      "id": 3,
      ...
      "updatedAt": "2026-09-09T14:15:00"
    }
    ```
- 미인증 접근 차단 검증
  - Auth 탭을 `No Auth`로 변경 후 동일한 게시글 등록 요청 전송 시, 401 Unauthorized 응답이 반환되며 비로그인 사용자의 게시글 작성이 원천 차단됨을 확인함.
