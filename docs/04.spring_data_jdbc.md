...
#### 4) 실행 계획 비교
- 전통적인 표(Grid) 방식 지표와 최신 트리(Tree) 방식 출력 문구의 비교

| 구분 | type (트리 문구 매핑) | possible_keys | key | rows | cost (예측 비용) | Extra (트리 문구 매핑) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 인덱스 미사용 (`like_count * 2 >= 1980`) | ALL (`Table scan on post`) | NULL | NULL | 약 500,000 | 53,515 | Using where (`Filter`), Using filesort (`Sort`) |
| 인덱스 정상 사용 (`like_count >= 990`) | range (`Index range scan...`) | idx_post_like_count | idx_post_like_count | 약 5,000 | 6,030 | Using index condition |
| 손익분기점 초과 (`like_count >= 500`) | ALL (`Table scan on post`) | idx_post_like_count | NULL | 약 500,000 | 53,515 | Using where (`Filter`), Using filesort (`Sort`) |

- 트리(Tree) 출력 문구 해석 기준
  - Table scan on 테이블명: `type = ALL`(풀 테이블 스캔)을 의미하며 인덱스를 타지 못한 상태.
  - Index range scan on 테이블명 using 인덱스명: `type = range`(인덱스 레인지 스캔) 및 `key = 인덱스명`을 의미하며 필요한 범위만 고속 탐색.
  - Sort: 컬럼명: `Extra = Using filesort`를 의미하며 인덱스 정렬 순서를 활용하지 못해 메모리/디스크에서 별도 정렬을 수행함.
  - Filter: 조건식: `Extra = Using where`를 의미하며 읽어온 행들에 대해 조건 검사를 수행함.

### 3.4.2 실행 계획 주요 분석 지표
- type (접근 방식)
...
- key: 쿼리 실행을 위해 실제로 채택되어 사용된 인덱스 명칭 (미사용 시 NULL).
- rows: 원하는 결과를 도출하기 위해 옵티마이저가 조사해야 할 것으로 예측한 대략적인 행 수 (ALL인 경우 전체 테이블 레코드 수).
- cost (실행 비용): 옵티마이저가 디스크 I/O(데이터 페이지 읽기)와 CPU 연산(비교, 필터링, 정렬) 부하를 종합하여 산출한 예측 비용 점수로, 이 수치가 가장 낮은 실행 계획이 최종 선택됨.
- Extra: 쿼리 처리 방식에 대한 추가적인 정보.
  - Using filesort: 인덱스 정렬을 활용하지 못하고 별도의 메모리/디스크 정렬 작업을 수행함 (개선 대상).
  - Using index condition: 인덱스 조건 푸시다운(ICP)을 적용하여 인덱스 엔진 레벨에서 필터링을 수행함.
...
```sql
-- 100000개의 게시글을 건너띄고 10개 조회(페이지당 10개씩 보여줄 경우 10001 페이지 조회)
