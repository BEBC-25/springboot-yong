...
- [1. 웹 보안 기초와 서블릿 필터 아키텍처](#1-웹-보안-기초와-서블릿-필터-아키텍처)
  ...
  - [1.3 Spring Security 서블릿 필터 체인 연동 구조](#13-spring-security-서블릿-필터-체인-연동-구조)
  - [1.4 Spring Boot 환경의 보안 설정 구성](#14-spring-boot-환경의-보안-설정-구성)
...
## 1.3 Spring Security 서블릿 필터 체인 연동 구조
- 서블릿 컨테이너의 필터는 서블릿 스펙에 의해 구동되므로, 원칙적으로 스프링의 IoC 컨테이너 [^8] 가 관리하는 빈(Bean)들을 직접 주입받아 활용하기 어려움.
- Spring Security는 이 이종 컨테이너 간의 결합 문제를 해결하기 위해 프록시 메커니즘을 적용함.

### 1.3.1 서블릿 컨테이너와 스프링 빈을 이어주는 DelegatingFilterProxy 메커니즘
...
### 1.3.2 보안 필터 목록을 총괄 제어하는 FilterChainProxy와 SecurityFilterChain 매핑 구조
...
- DSL [^9] 기반 SecurityConfig 설정 클래스 (src/main/java/net/likelion/bebc25/sns/security/config/SecurityConfig.java)
```java
...
- 주요 컴포넌트 역할
  - `Authentication`: 인증 주체의 신원 정보(Principal), 자격 증명(Credentials), 보유 권한 목록(Authorities)을 보관하는 최상위 인터페이스.
  - `AuthenticationManager`: 인증을 처리하는 핵심 인터페이스. 실무에서는 기본 구현체인 `ProviderManager`가 사용되며, 등록된 여러 `AuthenticationProvider` 목록을 순회하며 적합한 공급자에게 실제 인증 처리를 위임함.
  - `AuthenticationProvider`: 특정 인증 방식(예: 아이디/비밀번호 기반, OAuth2 기반, JWT 기반)을 전담하여 실제 자격 증명을 대조하고 인증된 객체를 생성함. 아이디/비밀번호 방식에서는 `DaoAuthenticationProvider`가 작동함.
  - `UserDetailsService`: 저장소(데이터베이스)에서 사용자 식별자(username)를 기반으로 회원 데이터(`UserDetails`)를 조회해 오는 단일 메서드 인터페이스.

### 2.1.2 Authentication 객체와 SecurityContextHolder 정보 관리 방식
#### 1) `SecurityContext`
- 역할: 현재 인증된 사용자 정보(`Authentication`)를 보관하는 보관함 인터페이스.
- 인증 객체 추출: `context.getAuthentication()`을 호출하여 저장된 `Authentication` 객체를 추출함.
- 인증 객체 저장: `context.setAuthentication(authentication)`을 통해 검증 완료된 인증 객체를 적재함.
- 세부 속성 접근: 반환된 `Authentication` 객체로부터 `getPrincipal()`(사용자 신원 객체/UserDetails), `getAuthorities()`(보유 권한 목록), `getName()`(사용자 식별자명)을 추출함.

#### 2) `SecurityContextHolder`
- 역할: `SecurityContext`를 담아두고 애플리케이션 전역 어디서든 접근할 수 있도록 정적(static) 메서드를 제공하는 헬퍼 클래스.
- `SecurityContext` 추출: `SecurityContextHolder.getContext()` 정적 메서드를 호출하여 현재 스레드에 바인딩된 `SecurityContext`를 꺼냄.
- `SecurityContext` 생성 및 저장: `SecurityContextHolder.createEmptyContext()`로 빈 `SecurityContext`를 생성하여 인증 객체를 설정한 뒤, `SecurityContextHolder.setContext(context)`로 스레드에 저장함.
- `SecurityContext` 정리: `SecurityContextHolder.clearContext()`를 호출하여 요청 처리가 끝난 스레드의 보안 정보를 초기화함.

#### 3) 저장 전략(SecurityContextHolderStrategy)
...
- `MODE_INHERITABLETHREADLOCAL`: 부모 스레드가 생성한 자식 스레드에도 보안 컨텍스트를 복사하여 전파함.
  - 별도의 새 자식 스레드에서 작업을 해야 할 경우 (결제나 주문 완료 후에 영수증 발행, 알림톡 전송, 외부 API 호출 등의 비동기 백그라운드 작업을 자식 스레드에서 처리해야 할 경우).
- `MODE_GLOBAL`: JVM 전역에서 하나의 정적 보안 컨텍스트를 공유함.
  - 데스크톱 독립 앱용.
...
### 2.3.5 @AuthenticationPrincipal 기반 현재 로그인 사용자 정보 주입
- 컨트롤러 계층에서 현재 인증된 사용자 정보를 간결하게 주입받기 위해 `@AuthenticationPrincipal` 어노테이션을 활용함.
...
public class MemberRestController {
    ...
    public ResponseEntity<MemberProfileResponse> getMyProfile(
        ...
        return ResponseEntity.ok(new MemberProfileResponse(memberId, email));
    }
}
```
...
### 3.1.1 authorizeHttpRequests DSL 및 RequestMatcher 매핑
- `authorizeHttpRequests` 내에서 순차적으로 매칭 조건을 검사함.
...
http.authorizeHttpRequests(auth -> auth
    ...
    // 특정 권한 보유자만 접근 가능한 경로
    .requestMatchers("/api/v1/admin/**").hasRole("ADMIN")
    .requestMatchers(HttpMethod.DELETE, "/api/v1/posts/**").hasAnyRole("ADMIN", "MANAGER")
...
### 3.1.2 permitAll, authenticated, hasRole, hasAuthority 차이
- `permitAll()`: 인증 여부와 무관하게 모든 사용자의 접근을 무조건 허용함.
- `authenticated()`: 익명 사용자가 아닌, 신원이 확인된 인증 사용자라면 권한에 상관없이 접근을 허용함.
- `hasAuthority("READ_PRIVILEGE")`: 부여된 권한 문자열이 정확히 일치하는지 검사함 (문자열 접두사를 자동으로 붙이지 않음).
- `hasRole("ADMIN")`: 역할 기반 접근 제어로, Spring Security는 내부적으로 `ROLE_` 접두사를 자동으로 붙여 검사함. 즉 `hasRole("ADMIN")`은 실제 사용자의 권한 목록에서 `ROLE_ADMIN`을 탐색함.

## 3.2 메서드 수준 보안
- URL 기반 인가는 컨트롤러 진입 단계에서만 동작하므로, 서비스 계층이나 컴포넌트의 특정 비즈니스 메서드 단위에서 세밀한 권한 검증을 수행하기 위해 메서드 보안을 사용함.

### 3.2.1 @EnableMethodSecurity 설정 활성화
- 보안 설정 클래스 상단에 어노테이션을 선언하여 스프링 AOP [^14] 프록시 기반의 메서드 보안 기능을 활성화함.
...
public class PostService {
    ...
    // 관리자이거나, 요청자가 해당 게시글의 작성자(memberId)인 경우에만 메서드 실행 허용
...
[^8]: IoC 컨테이너(Inversion of Control Container): 객체의 생성과 의존 관계 설정, 생명주기 제어권을 개발자가 아닌 프레임워크가 전담하여 제어의 역전을 실현하는 스프링의 핵심 런타임 엔진.
[^9]: DSL(Domain Specific Language): 특정 도메인(여기서는 웹 보안 설정)의 문제를 직관적이고 가독성 높게 기술할 수 있도록 문법을 최적화한 특화 언어로, 스프링 시큐리티에서는 메서드 체이닝과 람다 표현식으로 구성됨.
[^10]: CSRF(Cross-Site Request Forgery): 사용자가 로그인되어 있는 취약한 웹 사이트에 대해 공격자가 사용자의 브라우저를 유도하여 자신도 모르게 위조된 요청(비밀번호 변경, 송금 등)을 전송하게 만드는 웹 공격 기법.
