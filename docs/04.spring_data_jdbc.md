...
| 구분 | type (트리 문구 매핑) | possible_keys | key | rows | Extra (트리 문구 매핑) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 인덱스 미사용 (`like_count * 2 >= 1980`) | ALL (`Table scan on post`) | NULL | NULL | 약 500,000 | Using where (`Filter`), Using filesort (`Sort`) |
| 인덱스 정상 사용 (`like_count >= 990`) | range (`Index range scan...`) | idx_post_like_count | idx_post_like_count | 약 5,000 | Using index condition |
| 손익분기점 초과 (`like_count >= 500`) | ALL (`Table scan on post`) | idx_post_like_count | NULL | 약 500,000 | Using where (`Filter`), Using filesort (`Sort`) |
