...
### 5.3.9 SecurityConfig에 OAuth 2.0 로그인 등록
...
### 5.4.1 카카오 커스텀 provider 규격과 application-oauth.yaml 확장 (src/main/resources/application-oauth.yaml)
```yaml
spring:
  security:
    oauth2:
...
### 5.4.2 카카오 OAuth 2.0 클라이언트 자격증명 획득 절차
...
- 1단계: Kakao Developers 접속 및 애플리케이션 생성
  - 카카오 디벨로퍼스(https://developers.kakao.com/)에 로그인한 후 상단 '앱' > '+앱 생성' 버튼 선택.
...
### 5.4.3 카카오 다중 중첩 JSON 구조 및 비즈 앱 이메일 권한
- 카카오 프로필 응답 JSON 구조
  ```json
  {
    "id": 1234567890,
...
### 5.4.4 CustomOAuth2UserService 카카오 지원 확장
...
- `src/main/resources/static/oauth/login.html` 파일에 카카오 소셜 로그인 인가 요청 시작 링크 추가.
  ```html
  <!doctype html>
  <html lang="ko">
  <head>
