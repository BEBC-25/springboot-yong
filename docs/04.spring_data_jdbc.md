...
- 인덱스가 없는 컬럼을 조회할 때는 전체 테이블 데이터를 처음부터 끝까지 순차적으로 스캔하는 풀 테이블 스캔(Full Table Scan)이 발생함.
...
- `@Mapper` 인터페이스와 동적 프록시 (Dynamic Proxy) 작동 원리
  ...
  - 애플리케이션 기동 시 `mybatis-spring-boot-starter`가 인터페이스를 스캔하여 런타임에 동적 프록시 [^10] 구현 객체를 생성하고 스프링 빈(Bean)으로 컨테이너에 등록함.
...
### 4.3.1 매개변수 바인딩: `#{}` vs `${}`
- `#{}` (PreparedStatement 바인딩): 쿼리 실행 시 물음표(?) 파라미터로 변환되어 전달되므로 값에 따옴표가 자동 부착되고 SQL Injection [^11] 공격을 방지함.
...
- 트랜잭션(Transaction) [^12] 은 복수의 데이터베이스 처리 작업을 하나의 원자적 논리 단위로 묶어 무결성을 보장하는 기술.
- 스프링은 프록시 AOP [^13] 메커니즘을 기반으로 메서드 진입 시 트랜잭션을 시작하고, 성공 시 자동 커밋을, 런타임 예외(RuntimeException) 발생 시 자동 롤백을 수행함.
...
[^10]: 동적 프록시(Dynamic Proxy): 런타임 시점에 인터페이스를 기반으로 프레임워크가 구현 클래스 객체를 동적으로 생성하여 주입하는 기법
[^11]: SQL Injection: 입력 폼에 악의적인 SQL 문을 주입하여 데이터베이스를 비정상적으로 조작하거나 데이터를 탈취하는 공격 기법
[^12]: 트랜잭션(Transaction): 데이터베이스의 상태를 변경하는 작업들을 하나의 논리적 단위로 묶어 무결성을 보장하는 연산
[^13]: AOP(Aspect-Oriented Programming): 핵심 비즈니스 로직과 트랜잭션, 보안, 로깅 등 공통 관심사를 분리하여 모듈화하는 관점 지향 프로그래밍
