...
- 테스트용 정적 로그인 HTML 페이지 작성 (`src/main/resources/static/oauth/login.html`)
  - 스프링 부트는 `src/main/resources/static/` 경로에 위치한 정적 HTML 리소스를 서빙하므로, `http://localhost:8080/oauth/login.html` 경로로 접속.
...
- 단계별 실습 절차
  - 1단계: 스프링 부트 애플리케이션을 기동하고 웹 브라우저에서 `http://localhost:8080/oauth/login.html`에 접속.
