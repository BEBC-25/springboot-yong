...
- [3. 인덱스와 쿼리 성능 최적화](#3-인덱스와-쿼리-성능-최적화)
  - [3.1 인덱스 정의와 B-Tree 작동 원리](#31-인덱스-정의와-b-tree-작동-원리)
  - [3.2 쿼리 성능 저하 원인 분석](#32-쿼리-성능-저하-원인-분석)
  - [3.3 데이터베이스 실행 계획 확인](#33-데이터베이스-실행-계획-확인)
  - [3.4 페이징 처리 기법](#34-페이징-처리-기법)
- [4. MyBatis 연동](#4-mybatis-연동)
  - [4.1 MyBatis 구조와 스프링 부트 환경 설정](#41-mybatis-구조와-스프링-부트-환경-설정)
  - [4.2 Mapper 인터페이스와 XML 매퍼 연동](#42-mapper-인터페이스와-xml-매퍼-연동)
  - [4.3 ResultMap과 매개변수 바인딩](#43-resultmap과-매개변수-바인딩)
  - [4.4 동적 SQL 제어](#44-동적-sql-제어)
  - [4.5 MyBatis 기반 SNS 데이터 접근 계층 구현](#45-mybatis-기반-sns-데이터-접근-계층-구현)
...
- 중간 매핑 테이블을 통한 관계 해소 원리
  - 두 테이블 사이에 행위 개체 역할을 하는 중간 매핑 테이블(Junction / Bridge Table: `post_like`, `bookmark`)을 신설함.
...
## 3.1 인덱스 정의와 B-Tree 작동 원리

### 3.1.1 인덱스(Index) 정의
- 인덱스는 데이터베이스 테이블의 검색 속도를 높이기 위해 특정 컬럼 값을 정렬하여 주소 포인터와 함께 유지하는 별도의 검색 색인 자료구조.
- 인덱스가 없는 컬럼을 조회할 때는 전체 테이블 데이터를 처음부터 끝까지 순차적으로 스캔하는 풀 테이블 스캔(Full Table Scan) [^7] 이 발생함.

### 3.1.2 B-Tree(Balanced Tree) 구조와 동작
- 관계형 데이터베이스(MySQL InnoDB 등)는 인덱스의 기본 자료구조로 B-Tree(Balanced Tree) 구조를 채택함.
- B-Tree는 최상위 루트 노드(Root Node)에서 출발하여 브랜치 노드(Branch Node)를 거쳐 실제 데이터 행의 물리 주소(또는 PK)를 가진 리프 노드(Leaf Node)까지 균일한 깊이로 탐색을 수행함.
- 트리 탐색 시간 복잡도가 $O(\log N)$을 유지하므로 데이터양이 많아져도 검색 속도가 안정적임.

### 3.1.3 인덱스의 장단점
- 장점: SELECT 쿼리의 WHERE 조건 검색, ORDER BY 정렬, JOIN 연산 처리 속도가 개선됨.
- 단점: INSERT, UPDATE, DELETE 수행 시 인덱스 트리 구조도 실시간으로 재정렬해야 하므로 쓰기 연산 성능이 저하되며, 페이지 분할(Page Split) [^8] 현상 발생 시 추가 디스크 I/O 비용이 발생함. 또한 인덱스 저장을 위한 추가 디스크 공간(약 10~15%)이 소요됨.

## 3.2 쿼리 성능 저하 원인 분석

### 3.2.1 인덱스를 활용하지 못하는 부적절한 쿼리 패턴

#### 1) 인덱스 컬럼을 함수나 연산으로 가공하는 경우
- 인덱스 컬럼 원형을 변형하면 B-Tree 정렬 구조를 활용할 수 없어 풀 테이블 스캔이 유발됨.
  ```sql
  -- 부적절한 예시: 인덱스 컬럼에 DATE_FORMAT 함수 적용
  SELECT * FROM post WHERE DATE_FORMAT(created_at, '%Y-%m-%d') = '2026-06-12';

  -- 개선된 예시: 인덱스 컬럼을 가공하지 않고 범위 조건으로 비교
  SELECT * FROM post WHERE created_at >= '2026-06-12 00:00:00' AND created_at < '2026-06-13 00:00:00';
  ```

#### 2) LIKE 검색 시 와일드카드(%)를 앞부분에 사용하는 경우
- 검색어 시작 위치에 `%`가 붙으면 정렬 순서를 사용할 수 없어 풀 스캔으로 전환됨.
  ```sql
  -- 부적절한 예시: 앞단 와일드카드
  SELECT * FROM member WHERE email LIKE '%@gmail.com';

  -- 개선된 예시: 접두어 기반 검색
  SELECT * FROM member WHERE nickname LIKE 'lion%';
  ```

#### 3) 암시적 형 변환이 발생하는 경우
- 숫자형 컬럼에 문자열 값을 대입하여 비교하거나 반대로 처리할 때 데이터베이스 엔진이 내부적으로 형 변환 함수를 호출하여 인덱스를 무력화시킴.

## 3.3 데이터베이스 실행 계획 확인

### 3.3.1 EXPLAIN 명령어
- 쿼리문 앞에 `EXPLAIN` 키워드를 붙여 실행하면 옵티마이저 [^9] 가 수립한 실행 계획(Execution Plan) [^10] 세부 정보를 확인할 수 있음.

```sql
EXPLAIN SELECT * FROM post WHERE member_id = 10 ORDER BY id DESC;
```

### 3.3.2 실행 계획 주요 분석 지표
- type (접근 방식)
  - 쿼리 성능을 결정하는 가장 핵심적인 지표.
  - const: 기본 키 또는 유니크 키 조회를 통해 단 1건을 상수 수준으로 접근하는 가장 빠른 방식.
  - ref: 유니크가 아닌 일반 인덱스 등가(=) 조건 검색으로 우수한 성능.
  - range: 인덱스 범위 검색(BETWEEN, <, >, IN).
  - index: 인덱스 전체를 스캔하는 방식.
  - ALL: 인덱스를 타지 못하고 테이블 전체를 스캔하는 풀 테이블 스캔(성능 개선 대상).
- key: 쿼리 실행을 위해 실제로 채택되어 사용된 인덱스 명칭.
- rows: 원하는 결과를 도출하기 위해 옵티마이저가 조사해야 할 것으로 예측한 대략적인 행 수.

## 3.4 페이징 처리 기법

### 3.4.1 OFFSET 기반 페이징의 한계
...
### 3.4.2 No-Offset (커서 기반) 무한 스크롤 페이징 기법
...
-- 이후 무한 스크롤 요청 시 (직전에 읽은 마지막 post_id = 9520을 기준으로 다음 10건 조회)
SELECT id, content, image_url, member_id, created_at
FROM post
WHERE id < 9520
ORDER BY id DESC
...
- B-Tree 인덱스(id 기본키)의 특정 위치로 즉시 찾아가서 10건만 읽어오므로 데이터가 수천만 건에 달해도 일정한 조회 성능($O(1)$)을 보장함.

---

# 4. MyBatis 연동

## 4.1 MyBatis 구조와 스프링 부트 환경 설정

### 4.1.1 MyBatis 등장 배경과 SQL 매퍼의 개념
- SQL 중심 데이터 접근 기술의 발전 흐름
  - 순수 JDBC: 자바 소스코드 내에 SQL 문자열이 직접 하드코딩되고 `Connection`, `PreparedStatement`, `ResultSet` 등 반복적인 자원 획득/해제 보일러플레이트 코드가 과도하게 발생함.
...
| SQL 작성 위치 | 자바 소스코드 내부 문자열 | 독립된 XML 파일 또는 어노테이션 | 프레임워크가 자동 생성 (JPQL/메서드 쿼리) |
...
| 주요 적합 분야 | 단순 관리자 도구, 배치 스크립트 | 복잡한 통계 쿼리, 기존 레거시 SQL 유지보수 | 도메인 주도 설계(DDD), 비즈니스 로직 중심 |

### 4.1.2 MyBatis 핵심 아키텍처 및 내부 동작 메커니즘
- MyBatis 핵심 구성 요소
  - `SqlSessionFactory`: 설정 정보(`mybatis-config.xml` 또는 `application.yml`)와 매퍼 XML을 파싱하여 생성되는 싱글톤 팩토리 객체.
...
- `@Mapper` 인터페이스와 동적 프록시 (Dynamic Proxy) 작동 원리
  ...
  - 애플리케이션 기동 시 `mybatis-spring-boot-starter`가 인터페이스를 스캔하여 런타임에 동적 프록시 [^11] 구현 객체를 생성하고 스프링 빈(Bean)으로 컨테이너에 등록함.
...
### 4.1.4 스프링 부트 application.yml 설정

```yaml
mybatis:
  # 매퍼 XML 파일 위치 지정
  mapper-locations: classpath:mapper/**/*.xml
  # DTO/Record 모델 패키지 별칭 지정 (XML에서 전체 패키지명 생략 가능)
  type-aliases-package: net.likelion.bebc25.sns.domain
  configuration:
...
<!-- 인터페이스의 전체 패키지 경로를 namespace로 매핑 -->
<mapper namespace="net.likelion.bebc25.sns.mapper.PostMapper">

  <!-- 자동 증가 기본키(id) 반환 설정 (useGeneratedKeys, keyProperty) -->
  <insert id="insert" parameterType="Post" useGeneratedKeys="true" keyProperty="id">
    INSERT INTO post (content, image_url, member_id, created_at, updated_at)
    VALUES (#{content}, #{imageUrl}, #{memberId}, NOW(), NOW())
  </insert>
...
## 4.3 ResultMap과 매개변수 바인딩

### 4.3.1 매개변수 바인딩: `#{}` vs `${}`
- `#{}` (PreparedStatement 바인딩): 쿼리 실행 시 물음표(?) 파라미터로 변환되어 전달되므로 값에 따옴표가 자동 부착되고 SQL Injection [^12] 공격을 방지함.
- `${}` (문자열 직접 치환): 파라미터 값이 SQL 문장에 그대로 삽입되므로 테이블명이나 컬럼명을 동적으로 지정할 때를 제외하고는 보안상 사용을 지양해야 함.

### 4.3.2 복합 객체 조회를 위한 ResultMap 구성
...
### 4.4.2 SNS 피드 다중 검색 필터 동적 SQL 예시
...
## 4.5 MyBatis 기반 SNS 데이터 접근 계층 구현

### 4.5.1 좋아요 및 북마크 매퍼 인터페이스 (LikeBookmarkMapper.java)
...
@Mapper
public interface LikeBookmarkMapper {
    // 좋아요 토글 처리
    int insertLike(@Param("memberId") Long memberId, @Param("postId") Long postId);
    int deleteLike(@Param("memberId") Long memberId, @Param("postId") Long postId);
    boolean existsLike(@Param("memberId") Long memberId, @Param("postId") Long postId);
    int countLikesByPostId(@Param("postId") Long postId);

    // 북마크 처리
    int insertBookmark(@Param("memberId") Long memberId, @Param("postId") Long postId);
    int deleteBookmark(@Param("memberId") Long memberId, @Param("postId") Long postId);
    boolean existsBookmark(@Param("memberId") Long memberId, @Param("postId") Long postId);
}
```

### 4.5.2 서비스 계층 연동 (PostLikeService.java)
...
public class PostLikeService {
    ...
    public PostLikeService(LikeBookmarkMapper likeBookmarkMapper) {
        this.likeBookmarkMapper = likeBookmarkMapper;
    }

    @Transactional
    public boolean toggleLike(Long memberId, Long postId) {
        boolean alreadyLiked = likeBookmarkMapper.existsLike(memberId, postId);
        if (alreadyLiked) {
            likeBookmarkMapper.deleteLike(memberId, postId);
...
### 5.1.1 핵심 철학 및 특성
- Spring Data JDBC는 도메인 주도 설계(DDD, Domain-Driven Design)의 애그리거트(Aggregate) 개념을 지향하는 단순하고 가벼운 ORM 프레임워크.
...
- 트랜잭션(Transaction) [^13] 은 복수의 데이터베이스 처리 작업을 하나의 원자적 논리 단위로 묶어 무결성을 보장하는 기술.
- 스프링은 프록시 AOP [^14] 메커니즘을 기반으로 메서드 진입 시 트랜잭션을 시작하고, 성공 시 자동 커밋을, 런타임 예외(RuntimeException) 발생 시 자동 롤백을 수행함.
...
[^7]: 풀 테이블 스캔(Full Table Scan): 인덱스를 사용하지 않고 테이블의 모든 데이터 블록을 처음부터 끝까지 순차적으로 읽는 탐색 방식
[^8]: 페이지 분할(Page Split): 인덱스 리프 노드에 새로운 데이터를 삽입할 공간이 부족할 때 페이지를 분할하여 할당하는 디스크 I/O 작업
[^9]: 옵티마이저(Optimizer): 사용자 SQL 쿼리를 최소 비용으로 수행하기 위해 최적의 실행 경로를 분석하고 결정하는 데이터베이스 엔진 핵심 모듈
[^10]: 실행 계획(Execution Plan): 옵티마이저가 쿼리 처리를 위해 선택한 연산 순서와 데이터 접근 경로 명세서
[^11]: 동적 프록시(Dynamic Proxy): 런타임 시점에 인터페이스를 기반으로 프레임워크가 구현 클래스 객체를 동적으로 생성하여 주입하는 기법
[^12]: SQL Injection: 입력 폼에 악의적인 SQL 문을 주입하여 데이터베이스를 비정상적으로 조작하거나 데이터를 탈취하는 공격 기법
[^13]: 트랜잭션(Transaction): 데이터베이스의 상태를 변경하는 작업들을 하나의 논리적 단위로 묶어 무결성을 보장하는 연산
[^14]: AOP(Aspect-Oriented Programming): 핵심 비즈니스 로직과 트랜잭션, 보안, 로깅 등 공통 관심사를 분리하여 모듈화하는 관점 지향 프로그래밍
