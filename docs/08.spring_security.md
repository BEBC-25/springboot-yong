...
- 1단계 인증 요청 위임 (`AuthenticationManager` ➔ `AuthenticationProvider`)
  - `AuthenticationManager`의 기본 구현체인 `ProviderManager`는 여러 `AuthenticationProvider`를 리스트로 관리함.
  - 미인증 토큰(`UsernamePasswordAuthenticationToken`)을 전달받으면 각 Provider의 `supports()` 메서드를 호출하여 해당 토큰을 처리할 수 있는지 질의함.
  - 폼 로그인이나 아이디/비밀번호 기반 요청에 대해 처리 적합 판정을 내린 `DaoAuthenticationProvider`에게 `authenticate()`를 호출하여 실제 검증 작업을 위임함.
- 2단계 저장소 독립적 회원 데이터 조달 (`AuthenticationProvider` ➔ `UserDetailsService`)
  - `DaoAuthenticationProvider`는 자격 증명 검증을 위해 원천 회원 데이터가 필요하지만, 데이터베이스나 SQL/MyBatis에 직접 접근하지 않음.
  - 오직 `UserDetailsService` 인터페이스의 `loadUserByUsername(username)` 규약에만 의존하여 조회를 요청함.
  - 개발자가 구현한 `CustomUserDetailsService`가 DB에서 회원을 조회하여 `UserDetails` 규격 객체로 반환함으로써, 인증 엔진과 데이터 저장 계층 간의 결합도를 완벽히 격리함.
- 3단계 자격 증명 검증 및 인증 객체 발급 (`AuthenticationProvider` ➔ `PasswordEncoder`)
  - `DaoAuthenticationProvider`는 `UserDetailsService`로부터 전달받은 `UserDetails`의 암호화된 비밀번호와 클라이언트가 제출한 평문 비밀번호를 `PasswordEncoder.matches()`로 대조함.
  - 비밀번호 일치 여부와 함께 계정 만료, 잠김, 비활성화 등 부가 상태를 함께 검증함.
  - 모든 검증이 완료되면 사용자 식별자, 신원 정보(Principal), 승인된 권한 목록(Authorities)을 담은 새로운 '인증 완료 토큰'을 생성하여 `ProviderManager`를 거쳐 보안 필터로 되돌려줌.
...
- 사용 시점: 사용자 신원(`Principal`)뿐만 아니라 부여된 전체 권한 컬렉션(`getAuthorities()`), 인증 상태(`isAuthenticated()`) 등 보안 메타데이터를 종합적으로 검사해야 할 때 적합함.

```java
@GetMapping("/me/auth-info")
public ResponseEntity<String> getAuthInfo(Authentication authentication) {
    String email = authentication.getName();
    String roles = authentication.getAuthorities().toString();
    return ResponseEntity.ok("이메일: " + email + ", 보유 권한: " + roles);
}
```

#### 3) `SecurityContextHolder` 정적 조회를 통한 전역 접근 (서비스 및 공통 계층)
- 동작 메커니즘: 웹 컨트롤러 파라미터를 넘겨받지 못하는 서비스 계층, 엔티티 공통 감사(Audit) 리스너, 배치 작업 등에서 정적 메서드로 현재 스레드의 보안 컨텍스트에 직접 접근함.
...
|---|---|---|---|
| `@AuthenticationPrincipal` | 컨트롤러 계층 | 선언적이며 `CustomUserDetails` 타입으로 직접 다운캐스팅 바인딩 | `null` |
| `Authentication` 파라미터 | 컨트롤러 계층 | 권한 목록(`getAuthorities()`) 등 보안 메타데이터 종합 검사 가능 | `null` 또는 익명 토큰 |
...
- Bruno 설정
  - 주소창 하단 `Auth` 탭 클릭.
  - `Type` 드롭다운에서 `Basic Auth` 선택.
  - `Username`에 DB 등록 회원 이메일(`user@likelion.net`), `Password`에 평문 비밀번호(`Password123!`) 입력 후 Send 클릭.
...
- 1.5절의 `SecurityConfig`에 등록한 `AuthenticationManager`와 2장의 `CustomUserDetailsService` 인증 체계를 기반으로, 클라이언트의 로그인 요청을 처리하는 무상태 REST API 엔드포인트를 구축함.
